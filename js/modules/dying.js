...
const dyingModule = {
    async load() {
        if (!authManager.hasPermission('dying')) {
            document.getElementById('content-area').innerHTML = `
                <div class="alert alert-danger">
                    ...
                    <p>You don't have permission to access the Dying module.</p>
                </div>
            `;
            return;
        }
        ...
        const content = `
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4><i class="bi bi-palette"></i> Dying Process</h4>
                    <p class="text-muted">Manage cloth dying process and track costs</p>
                </div>
                <div>
                    <button class="btn btn-primary" onclick="dyingModule.showDyingForm()">
                        <i class="bi bi-plus-circle"></i> New Dying Process
                    </button>
                </div>
            </div>
            ...
            <div class="row mb-4">
                <div class="col-md-3">
                    <div class="card text-center">
                        <div class="card-body">
                            <h6 class="text-muted">Active Dying</h6>
                            <h3 id="active-dying">0</h3>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-3">
                            ...
                            <select class="form-select" id="filterStatus" onchange="dyingModule.filterDying()">
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-select" id="filterParty" onchange="dyingModule.filterDying()">
                                <option value="">All Parties</option>
                            </select>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Date From</label>
                            <input type="date" class="form-control" id="filterDateFrom" onchange="dyingModule.filterDying()">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Date To</label>
                            <input type="date" class="form-control" id="filterDateTo" onchange="dyingModule.filterDying()">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        <div>
                            <span id="dying-count">0</span> processes found
                        </div>
                        <nav>
        ...
        await this.loadDyingTable();
        await this.updateStats();
    },

    async initializeFilters() {
        try {
            const dyingParties = (await schema.findDocs('account')).filter(acc => 
                acc.account_type === 'dying_party' || acc.account_name.includes('Dying')
            );
            const partySelect = document.getElementById('filterParty');
            
            dyingParties.forEach(party => {
    ...
    async loadDyingTable(page = 1, itemsPerPage = 10) {
        try {
            const dyingProcesses = await schema.findDocs('process_order');
            const filteredProcesses = dyingProcesses.filter(p => p.process_type === 'dying');
            ...
            const html = paginated.map(process => {
                ...
                return `
                    <tr>
                        <td>
                            <div class="fw-bold">DYE-${process.process_number || process._id.substr(-6)}</div>
                        ...
                        <td>
                            <div class="btn-group btn-group-sm">
                                <button class="btn btn-outline-primary" onclick="dyingModule.viewProcess('${process._id}')">
                                    <i class="bi bi-eye"></i>
                                </button>
                                ${process.status !== 'completed' ? `
                                <button class="btn btn-outline-success" onclick="dyingModule.completeProcess('${process._id}')">
            ...
            document.getElementById('dying-table-body').innerHTML = 
                html || '<tr><td colspan="9" class="text-center">No dying processes found</td></tr>';
            
            document.getElementById('dying-count').textContent = total;
            this.generatePagination(page, totalPages);
            
        } catch (error) {
            console.error('Error loading dying processes:', error);
        }
    },
    ...
    generatePagination(currentPage, totalPages) {
        ...
        prevLi.innerHTML = `<a class="page-link" href="#" onclick="dyingModule.loadDyingTable(${currentPage - 1})"><i class="bi bi-chevron-left"></i></a>`;
        ...
        for (let i = 1; i <= totalPages; i++) {
            if (i === 1 || i === totalPages || (i >= currentPage - 2 && i <= currentPage + 2)) {
                ...
                li.innerHTML = `<a class="page-link" href="#" onclick="dyingModule.loadDyingTable(${i})">${i}</a>`;
        ...
        nextLi.innerHTML = `<a class="page-link" href="#" onclick="dyingModule.loadDyingTable(${currentPage + 1})"><i class="bi bi-chevron-right"></i></a>`;
    ...
    async updateStats() {
        try {
            const dyingProcesses = await schema.findDocs('process_order');
            const filtered = dyingProcesses.filter(p => p.process_type === 'dying');
            ...
            document.getElementById('active-dying').textContent = active;
            document.getElementById('month-dying').textContent = thisMonth;
    ...
    filterDying() {
        this.loadDyingTable(1);
    },

    async showDyingForm() {
        try {
            // Get available lots in gray stage
            const lots = (await schema.findDocs('lot')).filter(lot => 
                lot.current_stage === 'gray' && lot.status === 'active'
            );
            ...
            // Get available products for dying stage
            ...
            const partyOptions = dyingParties.map(party => 
            ...
            const formHtml = `
                <form id="dyingForm" onsubmit="return dyingModule.saveDying(event)">
                    <div class="form-section">
                        <h6 class="section-title">Select Lot</h6>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Lot *</label>
                                <select class="form-select" id="lotId" required onchange="dyingModule.loadLotDetails(this.value)">
                    ...
                    <div class="form-section">
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                <label class="form-label">Quantity to Dye *</label>
                    ...
                    <div class="d-grid">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-save"></i> Start Dying Process
                        </button>
                    </div>
                </form>
            ...
            app.showModal('New Dying Process', formHtml);
    ...
    async saveDying(event) {
        ...
        try {
            ...
            // Create dying process
            const processData = {
                process_type: 'dying',
                process_number: 'DYE-' + Date.now().toString().slice(-6),
                lot_id: lotId,
            ...
            await schema.createDoc('stock_movement', {
                ...
                qty: issuedQty,
                uom: processData.uom,
                reference_type: 'dying_issue',
                reference_id: process._id,
                movement_date: new Date().toISOString(),
                remarks: 'Issued for dying process',
                created_by: authManager.getCurrentUser().username
            });
            ...
            app.showToast('Dying process started successfully!', 'success');
            app.hideModal();
            await this.loadDyingTable();
            await this.updateStats();
            
        } catch (error) {
            console.error('Error saving dying process:', error);
        ...
        try {
            ...
            const content = `
                <div class="row">
                    <div class="col-md-6">
                        <table class="table table-bordered">
                            ...
                            <tr><th>Dying Party</th><td>${party?.account_name || 'N/A'}</td></tr>
                    ...
                    <div class="col-md-6">
                        <table class="table table-bordered">
                            ...
                            <tr><th>Created Date</th><td>${new Date(process.created_at).toLocaleString()}</td></tr>
                        </table>
                    </div>
                </div>
                ...
                <div class="d-grid gap-2 mt-3">
                    <button class="btn btn-success" onclick="dyingModule.completeProcess('${processId}')">
            ...
            app.showModal('Dying Process Details', content);
    ...
    async completeProcess(processId) {
        try {
            ...
            const receivedQty = prompt('Enter received quantity after dying:');
            ...
            await schema.createDoc('stock_movement', {
                ...
                uom: process.uom,
                reference_type: 'dying_complete',
                reference_id: process._id,
                movement_date: new Date().toISOString(),
                remarks: 'Dying process completed',
                created_by: authManager.getCurrentUser().username
            });
            
            app.showToast('Dying process completed successfully!', 'success');
            await this.loadDyingTable();
...
// Export the module
window.dyingModule = dyingModule;